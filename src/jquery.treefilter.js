		...
		el.find("li.tf-child-true").children("div").click(function(e) {
			// if (e.metaKey || e.ctrlKey) {
				if ($(this).parent().hasClass("tf-open")) {
					...
					$(this).parent().find("li.tf-child-false").removeClass("tf-selected");
				}
				else {
					if ($(this).parent().hasClass("pai")) {
						$('.tf-tree').find('li.tf-child-true').removeClass("tf-open");
						$('.tf-tree').find('li.tf-child-false').removeClass("tf-selected");
					}
	...
	var searcher = function() {
		$(plugin.settings.searcher).keyup(function() {
			if ($(this).val().length == 0) {
				plugin.el.find(".tf-search-result").parent().removeClass("tf-search-result");
				plugin.el.find(".tf-search-result").parent().removeClass("tf-open");
				...
				plugin.el.find("li:containsIN('" + $(this).val() + "')").addClass("tf-open");
				// plugin.el.find("li.tf-search-result").parent().addClass("tf-search-result");
				plugin.el.find("li.tf-search-result").parent().addClass("tf-open");
			}
		});
